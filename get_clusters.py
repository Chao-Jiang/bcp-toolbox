...
def extract_clusters(linkage_matrix, n, id_roi, data_directory, working_directory, subject):
    ...
    coords_roi = get_coords(working_directory, subject, coords_mat, id_roi)
    ...
    output_file = os.path.join(working_directory, subject,'k'+str(n)+'_roi'+str(id_roi)+'_clusters.txt')
...
if len(sys.argv) < 7:
    print ('usage: get_clusters <working_dir> <data_dir> <subjects_file> <id_roi_l> <id_roi_r> <max_nr_clusters')
else:
    # get directories
    ...
    with open(subjects_filepath, 'r') as subjects:
        ...
        for line in mylist:
            ...
            for i in xrange(2,max_n_clusters+1):
                # left
                extract_clusters(linkage_mat_l, i, idroi_l, data_dir, working_dir, subject)
                # right
                extract_clusters(linkage_mat_r, i, idroi_r, data_dir, working_dir, subject)
